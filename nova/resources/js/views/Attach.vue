<template>
  <loading-view :loading="loading">
    ...
    <!-- Phi -->
    <heading class="mb-3">{{
      attachHeadingLabel || __('Attach :resource', { resource: relatedResourceLabel })
    }}</heading>
    ...
    <form v-if="field" @submit.prevent="attachResource" autocomplete="off">
      <card class="overflow-hidden mb-8">
        ...
        <default-field :field="field" :errors="validationErrors">
          <template slot="field">
            ...
            <!-- Phi: class="form-control form-select mb-3 w-full" -->
            <select-control
              v-else
      ...
      <div class="flex items-center">
        ...
        >
          {{ confirmAttachAnotherButtonLabel || __('Attach & Attach Another') }}
        </progress-button>
        ...
        >
          {{
            confirmAttachButtonLabel || __('Attach :resource', {
              resource: relatedResourceLabel,
            })
          }}
        </progress-button>
...
export default {
  ...
  computed: {
    ...
    attachHeadingLabel() {
      if (this.field) {
        return this.field.attachHeadingLabel
      }
    },

    // Phi
    confirmAttachButtonLabel() {
      if (this.field) {
        return this.field.confirmAttachButtonLabel
      }
    },

    // Phi
    confirmAttachAnotherButtonLabel() {
      if (this.field) {
        return this.field.confirmAttachAnotherButtonLabel
      }
    },
