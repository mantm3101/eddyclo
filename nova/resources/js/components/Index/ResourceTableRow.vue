<template>
    <tr :dusk="resource['id'].value + '-row'">
        ...
        <td class="td-fit text-right pr-6">
            ...
            params: {
              ...
              resourceId: resource['id'].value,
                    ...
                    :title="__('View')"
                >
                    <icon :type="viaResource ? 'view' : 'edit'" />
                </router-link>
            </span>

            <span v-if="resource.authorizedToUpdate && viaResource">
                <!-- Edit Pivot Button -->
                <router-link
                    v-if="
          ...
          "
            ...
            <button
                ...
                v-if="
          viaResource && resource.authorizedToDelete &&
        ...
        "
            ...
            >
                <!-- Phi added: :resource-name="resourceName" -->
                ...
                >
                    <div slot-scope="{ uppercaseMode, mode }" class="p-8">
                        ...
                        >{{ __(`Are you sure you want to ${mode} this :resource?`, {resource: singularName.toLowerCase()}) }}</p>
                    </div>
                </delete-resource-modal>
                ...
                >
                    <div class="p-8">
                        <heading :level="2" class="mb-6">
                            {{
                            __('Restore :resource', {resource: singularName})
                            }}
                        </heading>
                        <p
                            class="text-80 leading-normal"
                        >{{ __('Are you sure you want to restore this :resource?', {resource: singularName.toLowerCase()}) }}</p>
                    </div>
                </restore-resource-modal>
            </portal>
        </td>
